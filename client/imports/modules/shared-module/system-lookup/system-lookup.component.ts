...
export class SystemLookupComponent implements OnInit, OnChanges, OnDestroy {
  ...
  async runActions() {
    ...
    if (this.config.enableMultipleUsersUpdate) {
      ...
      await Promise.all(this.methodsToBeRun.map(async (method: any) => {
        let result = await funcs.callbackToPromise(MeteorObservable.call(method.type, method.collectionName, method.query, method.update, method.options));

        if ('log' in method) {
          this.systemLogsService._log$(method.log).subscribe();
        }
  ...
  async PDF() {
    this.loading = true;
    let allResults = await this.lookup._getQueryAggregateResult().catch(error => console.log(error));
    let totals = await this.lookup._calculateTotal().catch(error => console.log(error));
    let pdfContent = {
      lookup: this.lookup,
      result: allResults
    };
    ...
    let docDefinition = await pdfFuncs.reportPdf(pdfContent, this.groupByPDF, this.sortByPDF).catch(error => console.log(error));
