...
export class InventoryWorkOrderReleaseDetailPage implements OnInit, OnDestroy {
      ...
      if (this.productionOrder) {
        ...
        productionOrder._save$()
          .subscribe(() => {
            ...
            let docDefinition:any = this.inventoryService._getPDFData(this.productionOrder);
        ...
        MeteorObservable.call('findOne', "systemOptions", {
          ...
          .subscribe(() => {
            let docDefinition:any = this._getPDFData(model);
            pdfMake.createPdf(docDefinition).open();
  ...
  _getPDFData(model) {
    ...
    return {
      ...
      content: [
        { text: 'Production Order Picking Sheet', style: 'header' },
        {
          margin: [0, 20, 0, 0],
          columns: [
            {
