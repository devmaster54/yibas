...
export class CustomersContractsUpdatePage implements OnInit, OnDestroy {
  ...
  async update() {
    ...
    if (this.increasePercentage != 0) {
          ...
          if (contractId && contractId != '') {
            funcs.getContractProductsById(contractId).then(async (contractProducts:any) => {
              ...
              if (needUpdate) {
                ...
                // console.log('count', count, contractId, numberProducts);
                // await Promise.all(logs.map(async (_log) => {
                //   await this.systemLogService._log$(_log).toPromise();
                //   return true;
                // }));
      ...
      // let result1 = await this.systemLogService._multiLogs(logs).toPromise();
      //
      // console.log('one update', result1);


      // MeteorObservable.call('rawInsert', 'systemLogs', []);
      ...
      //
      // MeteorObservable.call('rawInsert', 'systemLogs', logs)
      //   .subscribe(res => {
      //     console.log("res", res);
      //   });
      // funcs.update('customerContracts', {_id: contractId}, update).then((res:any) => {
      //   count++;
      //   this.progressPercentage = count/length * 100;
      //   resolve(res);
      // });
