...
function func1(pdfContent, groupArray, sortArray) {
  if (sortArray.length > 0) {
    pdfContent.result = normalSort(pdfContent.result, sortArray)
  }
    ...
    rowSpecificInfo.forEach(rowInfo => {
      ...
      if (!_.contains(hiddenColumns, rowInfo.prop)) {
        ...
        if ('_id' in element && element['_id'] === '000') {
          if (rowInfo.total) {
            row.push({ text: element[rowInfo.prop].toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","), fontSize: 9, bold: true, alignment: rowInfo.alignment, margin: [0, 0, 0, 0], border: [false, true, false, true] })
          } else {
            row.push({ text: '', alignment: rowInfo.alignment, bold: true, fontSize: 9, margin: [0, 0, 0, 0], border: [false, false, false, false] })
          }
        } else {
...
function groupAndMap(items, itemKey, childKey, predic?) {
  // console.log(items, itemKey, childKey);
  let grouped = _.map(
...
function checkType(text) {
  // console.log(text instanceof Date);
  // console.log(text instanceof Date ? moment(text).format("MM/DD/YYYY") : text);
...
function groupFunctions(pdfContent, groupOrderArray){
  let result
