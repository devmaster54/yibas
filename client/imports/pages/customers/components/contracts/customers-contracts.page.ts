...
export class CustomersContractsPage implements OnInit, OnDestroy{
        ...
        switchMap((contract:any) => {
          if (contract) {
            ...
            contract.forEach(res => {
              data.push([res.categoryDescription, 'Customer Part No.', 'Global Part No.', 'Description', 'Price/each', 'Case Qty']);
    ...
    if (row[checkboxFieldName]) {
      ...
      if (row[column.prop] != 0 && row[column.prop] != undefined) {
        ...
        if (
          ...
          // select
          if (initRow[checkboxFieldName]) {
            // if init row is checked, just update the price
            this.setPrice(event, row, column);
          } else {
            // if init row is not checked, insert the new product with the price
            this.insertContractProduct(event, row, column);
          }
        } else {
  ...
  removeContractProduct(row) {
    ...
    let method = {
      ...
      rowId: row._id,
      collectionName: 'customerContracts',
      query: { _id: this.state.contractId },
      update: update,
      type: 'update.remove',
      fieldPath: `products_${row._id}`,
      log: ''
  ...
  setPrice(event, row, column) {
    ...
    let newPrice = Number(row[column.prop].toFixed(2));
    let previousPrice;
    let update, method;
    if (originalRow.isOnContract) {
      if (!originalRow['price']) {
        ...
        previousPrice = Number(originalRow['price'].toFixed(2));
        update = {
          $set: {
            "products.$.price" : newPrice,
