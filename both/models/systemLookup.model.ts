...
export class SystemLookup implements SystemLookupModel {
  ...
  async _runQueryAggregateMethod(method) {
    let methodArgs = this._getMethodArgs(method);
    if (!this.hasNextMethod(method)) {
      // no next method
  ...
  async _getQueryAggregateResult() {
    let returnResult:any;
    ...
    while(index < aggregateMethods.length) {
      ...
      let aggregateResult = await this._runQueryAggregateMethod(aggregateMethods[index]).catch(error => console.log(error));
